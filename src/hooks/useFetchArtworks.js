...
export const useFetchArtworks = () => {
    ...
    const fetchArtworks = async () => {
      try {
        const [staticResponse, animatedResponse] = await Promise.all([
          axios.get('https://demo-project-backend-ude8.onrender.com/api/artworks/static', {
            headers: { Authorization: `Bearer ${localStorage.getItem('access_token')}` },
          }),
          axios.get('https://demo-project-backend-ude8.onrender.com/api/artworks/animated', {
            headers: { Authorization: `Bearer ${localStorage.getItem('access_token')}` },
          }),
        ]);
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(`https://demo-project-backend-ude8.onrender.com/api/artworks/${id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('access_token')}` },
      });
